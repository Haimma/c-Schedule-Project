...
namespace Schedule
{
    public partial class LecturerPreferences : Form
    {
        List<Panel> listPanel = new List<Panel>();
        int index, Semester, StartDay, EndDay, DaysCount;
        CoursesDB Lec = new CoursesDB();
        LecturerPreferencesDB LPreferences = new LecturerPreferencesDB();
        LecturerToCoursesDB LecCourses = new LecturerToCoursesDB();
        char A,B,C;
        string Teach, ID;
        public LecturerPreferences(string IDuser)
        {
            InitializeComponent();
            ID = IDuser;
        }
        ...
        private void LecturerPreferences_Load(object sender, EventArgs e)
        {
        ...
        {
            LecCourses.ClearByID(ID);
            foreach(string temp in listBox4.Items)
            {
                LecCourses.AddLecturerCourses(ID,temp);
            }
            LPreferences.ClearByID(ID);
            LPreferences.AddLecturerPreferences(ID, DaysCount, Teach, StartDay, EndDay);
        }
